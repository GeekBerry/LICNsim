...
class DataBaseTable:
    ...
    def __init__(self, *primary_keys, **value_dict):
        """
        创建一个数据库表
        :param primary_keys: [str, ...] 主键列表
        :param value_dict: 其他域字典

        >>> db_t= DataBaseTable('k1', 'k2', v1=0) # 不带默认值的为主键;  k1, k2 为主键
        """
        self.data_base= PyDataBase(path=':memory:')
        self.data_base.create(*primary_keys, *list(value_dict.items()),mode= self.MODE)
        self.data_base.create_index(*primary_keys)

        self.primary_keys= primary_keys
        self.key_table= {}  # {primary_key:id}

    def create_index(self, *fields):
        self.data_base.create_index(*fields)
    ...
    def __insert(self, key, field_dict=None):
        ...
        # 插入和注册
        r_id= self.data_base.insert(**field_dict)
    ...
    def __update(self, record, field_dict):
        ...
        self.data_base.update(record, **field_dict)

    def _setitem(self, record, key, value):  # 设置 record 的一个属性值
        if key in self.primary_keys:
            raise ValueError(f'{key} 是主键, 不能修改')

        if key in self.data_base.indices:
            self.data_base.update(record, **{key:value})
    ...
    def query(self, **condition_dict):
        ...
        """
        # 对有索引的项, 进行集合析取
        index_fields = condition_dict.keys() & set(self.data_base.indices.keys())
        ...
        if find_ids is None:
            find_ids= self.data_base.records.keys()
        ...
        for record in map(self.data_base.records.__getitem__, find_ids):
    ...
    def _filterIndexedIds(self, field, condition)->set:
        ...
        if callable(condition):
            find_ids= set()
            for var, ids in self.data_base.indices[field].items():
                if condition(var):
                    find_ids.update(ids)
            return find_ids
        else:  # condition 是一个值, 直接查找对应ids, 并转化为set
            return set( self.data_base.indices[field].get(condition, []) )
