...
ICNNetHelper.node(graph_info.graph, graph_info.center).api['CS::store'](debug.debug_dp)  # 要在CS类型配置之后,才会被正确驱逐
# 请求发生器
ask_gen= AskGenerator(graph_info.graph, FixedAsk(1), UniformPosition(graph_info.graph), debug.debug_ip, delta=4 * graph_info.diameter)
ask_gen.start(0)
# -----------------------------------------------------------------------------
