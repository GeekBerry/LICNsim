...
# ----------------------------------------------------------------------------------------------------------------------
class NameTree:
    value_count= 0  # 记录一个 NameTree 节点及所有子节点中, 有效节点数量; 在 'value' 增删时会更新
    ...
    def name(self):
        ...
        return Name([ name_node.key for name_node in forebear_list[::-1] ])
    ...
    def delValue(self):
        ...
        for each in self.forebears():  # 注: forebears 包含自身
            each.value_count -= 1
            if each.value_count == 0:  # 找到最前一个 value_count 为 0 的祖先
                cut_node = each
...
class NameTable:
    ...
    """
    def __init__(self, NameTreeType= NameTree):
        self.name_tree = NameTreeType()
